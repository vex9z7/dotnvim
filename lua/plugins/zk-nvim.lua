local function setup_mappings()
  ...
  local function searchByTags()
    return zk.pick_tags({ sort = { 'note-count-' } }, {}, function(selectedItems)
      local selectedTags = {}
      ...
      if finderType ~= nil then
        if finderType == 'tags' then
          ...
          if finderOption then
            local opts = vim.tbl_extend('keep', finderOption, { sort = defaultSortOption })
            return zk.edit(opts)
          end
        end
      end
  ...
  local function newNote()
    -- FIXME:  the ui turns weird after migration
  ...
  local function extratLinesToNote()
    ...
    -- get selectedContent
    local lines = vim.fn.getregion(visualStartPosition, visualEndPosition, { type = vim.fn.mode() })
  ...
  -- find notes
  vim.keymap.set({ 'n' }, '<leader>szv', findNote, { noremap = true, desc = '[s]earch in the [z]k [v]ault' })

  -- new notes
  vim.keymap.set({ 'n' }, '<leader>zn', newNote, { noremap = true, desc = 'Create a new [z]k [n]ote' })

  -- show backlinks
  vim.keymap.set({ 'n' }, '<leader>szb', function()
